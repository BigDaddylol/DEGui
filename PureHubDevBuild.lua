...
local guiTitle = "Pure Hub"

if game.CoreGui:FindFirstChild(guiTitle) then
...
local Main = library:CreateMain({
	-- Required
	projName = guiTitle, -- The name for the UI in game.CoreGui, not the GUI itself.
	-- Optional
	Resizable = true, -- Requires MinSize and MaxSize
	MinSize = UDim2.new(0, 750, 0, 400),
	MaxSize = UDim2.new(0, 750, 0, 459)
})

game.CoreGui[guiTitle].Motherframe.Size = UDim2.new(0, 750, 0, 459)

-- Categories

local categ1 = Main:CreateCategory(guiTitle)
local categ2 = Main:CreateCategory("General Scripts")
local categ3 = Main:CreateCategory("Game loader")
...
-- Credits
local cred0 = sect1:Create("Textlabel", "Script Version: 2.0.2 (Dev build)")
...
local aimbotIG = sect3:Create("Button", "Load Aimbot",
	function()
...
local aimbottoggler = sect3:Create("Keybind", "Aimbot toggle",
	function()
		if aimb == true then
			aimb = false
			MB2Held = false
        	MouseTests()
		elseif aimb == false then
			aimb = true
			MB2Held = true
        	MouseTests()
		end
...
local noclipToggler = sect3:Create("Keybind", "Noclip Toggle", -- The name of the keybind
    function()
        ...
        if noClipState then
			...
			if char:FindFirstChild'Torso' and char:FindFirstChild'Head' then
				char:FindFirstChild'Torso'.CanCollide = true
...
local function drivinEmpir()
	...
	local drvnemprautofarm = sect5:Create("Button", "AutoFarm",
		function()
			...
			Connection = game:GetService("RunService").Stepped:Connect(function()
				...
				if SeatPart.Position.X > -14294.7607 then
					...
					Vehicle:MoveTo(Vector3.new(-17157.2793, 83.1872673, 2641.22705))
					Character.Parent = workspace
				end
				if Speed < 550 then 
					local WV = GetWheelVelocity()
					SetWheelVelocity(Vector3.new(deautofrmspeed, WV["Y"], deautofrmspeed / -50.1940043308)) 
				end
			end)
	...
	pmpCntr = sect5:Create("Textlabel", "There are currently still processing pumpkins.")
	...
	local tpthingtp = sect5:Create("Button", "Tp to selected location",
		function()
			if selloctn == "Cars & Mortorcycles Dealer" then
				Tpchar(CFrame.new(-246.507, 26, -961.918))
			elseif selloctn == "City Dealer" then
				Tpchar(CFrame.new(-1736, 14.2, 2515))
			elseif selloctn == "Boat Dealer" then
				Tpchar(CFrame.new(-1471.5, 14.2, 2312))
			elseif selloctn == "Helocopter & Plane Dealer" then
				Tpchar(CFrame.new(165, 14.2, 2821))
			elseif selloctn == "Around The City Race" then
				Tpchar(CFrame.new(-2554, 14.2, 3588))
			elseif selloctn == "Circuit Race" then
				Tpchar(CFrame.new(-5546, 15, -4358))
			elseif selloctn == "City 1/4 Mile" then
				Tpchar(CFrame.new(-1361, 14.2, 4657))
			elseif selloctn == "Drag Race" then
				Tpchar(CFrame.new(-3813, 12.6, 6886))
			elseif selloctn == "Highway Race" then
				Tpchar(CFrame.new(-52, 35, -119))
			elseif selloctn == "Mountain Dash" then
				Tpchar(CFrame.new(4388, 365.7, -2782))
			end
		end,
		{
			animated = true
...
function onRenderStep()
	...
	if game.PlaceId == 3351674303 then
		local children = game:GetService("Workspace").Game.Halloween.Pumpkins:GetChildren()
		local pumpkins = #children
		pmpCntr:SetText("There are currently "..tostring(pumpkins).." pumpkins.")
	end
end
